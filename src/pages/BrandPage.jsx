...
import brandReportStyles from "@/util/stylePdf/PdfReportStyles";
import {
...
export default function BrandPage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      let res;
      if (isEdit) {
        res = await request(`brands/${form.id}`, "put", form);
      } else {
        res = await request("brands", "post", form);
      }
      if (res?.error) {
  ...
  const onDelete = async () => {
    ...
    try {
      await request(`brands/${deleteItem.id}`, "delete");
      fetchingData();
    } finally {
      setLoading(false);
  ...
  const exportToExcel = () => {
    ...
    const dataToExport = brands.map((item, index) => ({
      N: index + 1,
      Name: item.name,
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Brands List");

    const excelBuffer = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    saveAs(new Blob([excelBuffer]), "brands_list.xlsx");
  };
  const importFromExcel = (e) => {
    ...
    reader.onload = async (evt) => {
      ...
      try {
        const res = await request("brands/bulk", "post", { brands: mappedData });
        if (res) fetchingData();
      } catch (error) {
  ...
  const MyDocument = ({ data }) => (
    <Document>
      <Page size="A4" style={brandReportStyles.page}>

        {Logo && <Image style={brandReportStyles.logo} src={Logo} />}

        <Text style={brandReportStyles.header}>Mart POS System</Text>
        <Text style={brandReportStyles.subHeader}>Brand Report</Text>

        <View style={brandReportStyles.table}>
          <View style={brandReportStyles.tableRow}>
            {["No", "Name", "Description", "Created", "Updated", "Creator"].map((col, i) => (
              <Text
                key={i}
                style={[
                  brandReportStyles.tableHeader,
                  i === 5 && brandReportStyles.tableHeaderLast
                ]}
              >
                {col}
              </Text>
          ...
          {data.map((item, index) => (
            <View
              key={index}
              style={[
                brandReportStyles.tableRow,
                index % 2 === 0 && brandReportStyles.rowEven
              ]}
            >
              <Text style={brandReportStyles.tableCell}>{index + 1}</Text>
              <Text style={brandReportStyles.tableCell}>{item.name}</Text>
              <Text style={brandReportStyles.tableCell}>{item.description}</Text>
              <Text style={brandReportStyles.tableCell}>{formatDate(item.created_at)}</Text>
              <Text style={brandReportStyles.tableCell}>{formatDate(item.updated_at)}</Text>
              <Text style={[brandReportStyles.tableCell, brandReportStyles.tableCellLast]}>
        ...
        <Text style={brandReportStyles.footer}>Generated by Mart POS System</Text>

      </Page>
    </Document>
  );
  const downloadPDF = async () => {
    const blob = await pdf(<MyDocument data={brands} />).toBlob();
    saveAs(blob, "brands_report.pdf");
  };
  const header_table = [
    "N",
    "Name",
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6 w-full">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-muted-foreground mt-2">Manage your product brands.</p>
      </motion.div>
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center gap-4 w-full">
          ...
          <Button
            ...
            onClick={async () => {
              try {
                const res = await request(`brands/search/?q=${query}`, "get");
                if (res) setBrands(res.data);
              } catch (error) {
                console.error(error);
              }
          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>Fill in the brand information.</DialogDescription>
              </DialogHeader>
